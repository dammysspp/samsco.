...
function renderVaultSidebarNav() {
    ...
    const catIcons = {
        ...
        "palette": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01"/></svg>`,
        ...
        "sparkles": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>`,
        "code": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>`,
        "play": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`,
        "film": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>`,
        "presentation": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"/></svg>`,
        "mic": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 100-6 3 3 0 000 6z"/></svg>`,
        ...
        "globe": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>`,
        "tag": `<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>`,
    ...
    function resolveSidebarIcon(catName) {
        if (catIcons[catName]) return catIcons[catName];
        const lower = (catName || "").toLowerCase();
        if (lower.includes("3d") || lower.includes("vfx")) return catIcons["cube"];
        if (lower.includes("ai")) return catIcons["sparkles"];
        if (lower.includes("code") || lower.includes("data")) return catIcons["code"];
        if (lower.includes("motion")) return catIcons["play"];
        if (lower.includes("video") || lower.includes("film")) return catIcons["film"];
        if (lower.includes("presentation")) return catIcons["presentation"];
        if (lower.includes("radio") || lower.includes("audio") || lower.includes("sound")) return catIcons["mic"];
        if (lower.includes("social")) return catIcons["users"];
        if (lower.includes("web") || lower.includes("site") || lower.includes("app")) return catIcons["globe"];
        return catIcons["palette"];
    }
    ...
    categoriesToRender.forEach(cat => {
        ...
        const iconSvg = resolveSidebarIcon(cat);
...
let reelsContainerDelegated = false;

function initReelsContainerDelegation() {
    const reelsContainer = document.getElementById("mobile-reels-container");
    if (!reelsContainer || reelsContainerDelegated) return;
    reelsContainerDelegated = true;

    let lastTap = 0;
    reelsContainer.addEventListener("click", (e) => {
        const closeBtn = e.target.closest(".reel-close-btn");
        if (closeBtn) {
            e.stopPropagation();
            closeProjectModal();
            return;
        ...
        if (likeBtn) {
            e.stopPropagation();
            const slide = likeBtn.closest(".reel-slide");
        ...
        if (shareBtn) {
            e.stopPropagation();
            const slide = shareBtn.closest(".reel-slide");
            const configIndex = slide ? parseInt(slide.getAttribute("data-config-index")) : 0;
            const work = galleryConfig[configIndex];
            const shareData = {
                title: work?.title || "Samsco Creative Work",
                text: `Check out ${work?.title || 'this work'} on Samsco Portfolio Vault`,
        ...
        if (moreBtn) {
            e.stopPropagation();
            const descEl = moreBtn.parentElement ? moreBtn.parentElement.querySelector(".reel-desc-text") : null;
            if (descEl) {
        ...
        if (link) {
            const slide = link.closest(".reel-slide");
            if (slide) {
                const configIndex = parseInt(slide.getAttribute("data-config-index"));
                const work = galleryConfig[configIndex];
                if (work) trackWorkInteraction(work, "clicks");
            }
            return;
        }
        ...
        if (slide && !e.target.closest("button") && !e.target.closest(".reel-action-btn") && !e.target.closest(".reel-ba-container") && !e.target.closest("#reel-process-sheet")) {
            ...
            if (now - lastTap < 300) {
                ...
                const slideLikeBtn = slide.querySelector(".reel-like-btn");
                if (slideLikeBtn) slideLikeBtn.click();
            } else {
        ...
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting && entry.intersectionRatio >= 0.5) {
                ...
                slideVideos.forEach(video => {
                    ...
                    if (!video._bufferHooked) {
                        video._bufferHooked = true;
                        ...
                        const handleVideoReady = () => {
                            ...
                            if (spinnerEl && spinnerEl.parentNode) {
                                ...
                                setTimeout(() => { if (spinnerEl.parentNode) spinnerEl.remove(); }, 200);
                            }
                            if (posterEl) posterEl.style.opacity = "0";
                        };
                        video.addEventListener("playing", handleVideoReady, { passive: true });
                        video.addEventListener("play", () => {
                            trackWorkInteraction(work, "plays");
                        }, { passive: true });
                        video.addEventListener("timeupdate", () => {
                            if (video.currentTime > 0) handleVideoReady();
    ...
    if (!ticking) {
        window.requestAnimationFrame(() => {
            if (scrollProgressBar) {
                ...
                const progress = maxScroll > 0 ? (window.pageYOffset || document.documentElement.scrollTop) / maxScroll : 0;
...
if (backToTopBtn) {
    window.addEventListener("scroll", () => {
        ...
        if (shouldBeVisible !== isBackToTopVisible) {
            ...
            backToTopBtn.classList.toggle("visible", isBackToTopVisible);
        }
    }, { passive: true });
...
if (aboutImgContainer && window.matchMedia("(min-width: 768px)").matches) {
    ...
    s && (s.style.transition = "transform 0.2s ease-out", aboutImgContainer.addEventListener("mousemove", e => {
        ...
        s.style.transform = `perspective(1000px) rotateX(${l}deg) rotateY(${n}deg) scale(1.02)`;
...
const isTouchOrMobileDevice = (typeof isMobile !== "undefined" && isMobile) || (window.matchMedia && window.matchMedia('(hover: none) and (pointer: coarse)').matches) || window.innerWidth < 768;
if (canvas) {
    if (isTouchOrMobileDevice) {
        ...
        for (let h = 0; h < p; h++) g.push(new m);
